            ...
            $.post(url, async (err, resp, data) => {
                try {
                    ...
                    if (data.data.bizCode === 0) {
                        ...
                        let oldCodeArr = JSON.parse($prefs.valueForKey("闪购盲盒") || '[]');
                        oldCodeArr.push(invites && invites[0]['assistTaskDetailVo']['taskToken']);
                        $prefs.setValueForKey(JSON.stringify(oldCodeArr), "闪购盲盒");
                    }
                } catch (e) {
                    $.logErr(e, resp);
                } finally {
